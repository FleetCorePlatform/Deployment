...
- **Why IoT Jobs?** We use **IoT Jobs** to deploy missions. This ensures that even if a drone is temporarily offline, it will receive the mission command as soon as it reconnects.
- **Scheduling:** The system utilizes **Job Scheduling** to allow coordinators to plan missions that start at a specific future time. The job lifecycle (PENDING -> IN_PROGRESS -> SUCCEEDED) provides built-in status tracking.
...
- **Data Channels:** We utilize **WebRTC Data Channels** to send low-latency manual flight commands from the `FleetCoreDesktop` to the `OnboardAgent`.
- **Why WebRTC?** It bypasses the server entirely for both video and control data, ensuring the lowest possible latency for manual flight operations.
...
- **Lambda (process-detection):** Triggered by the AWS IoT routing engine when a drone publishes a **detection message** (via AI/ML processing). It populates the database with the detection metadata and S3 image reference, ensuring the main `FleetCoreServer` remains stateless by offloading database writes.
